        ...
        <div style="padding: 1rem; border-bottom: 1px solid #ecf0f1; display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <p style="color: #7f8c8d; font-size: 0.9rem; margin-top: 0.3rem;">
                    <span class="branch-badge branch-${(booking.branch || 'Kampala').toLowerCase()}" style="font-size: 0.75rem; padding: 0.2rem 0.6rem;">${booking.branch || 'Kampala'}</span>
...
function displayBookingsTable(bookingsToShow) {
    ...
    tbody.innerHTML = bookings.map(booking => `
        <tr>
            <td>#${booking.id}</td>
            <td><span class="branch-badge branch-${(booking.branch || 'Kampala').toLowerCase()}">${booking.branch || 'Kampala'}</span></td>
            <td>${booking.name}</td>
            <td>${booking.email}</td>
            <td>${booking.phone}</td>
...
function viewBooking(bookingId) {
    ...
    modalBody.innerHTML = `
        ...
        <div class="booking-detail">
            ...
            <p><span class="branch-badge branch-${(booking.branch || 'Kampala').toLowerCase()}">${booking.branch || 'Kampala'}</span></p>
        </div>
        <div class="booking-detail">
...
function sendWhatsAppNotification(booking, action) {
    ...
    const branchName = booking.branch || 'Kampala';
    
    if (action === 'approved') {
        message = `✅ *BOOKING CONFIRMED* ✅\n\n` +
                  `Dear ${booking.name},\n\n` +
                  `Your booking at Paradise Hotel ${branchName} has been APPROVED!\n\n` +
                  `📋 *Booking Details:*\n` +
                  ...
                  `Paradise Hotel ${branchName} 🏨`;
    } else {
        message = `❌ *BOOKING CANCELLED* ❌\n\n` +
                  `Dear ${booking.name},\n\n` +
                  `Unfortunately, your booking at Paradise Hotel ${branchName} has been cancelled.\n\n` +
                  `📋 *Booking Details:*\n` +
                  `Booking ID: #${booking.id}\n` +
                  `Hotel Branch: ${branchName}\n` +
...
function sendEmailNotification(booking, action) {
    ...
    if (action === 'approved') {
        subject = `Booking Confirmed - Paradise Hotel ${branchName} - Booking #${booking.id}`;
        body = `Dear ${booking.name},\n\n` +
               `Great news! Your booking at Paradise Hotel ${branchName} has been CONFIRMED!\n\n` +
               `BOOKING DETAILS:\n` +
               `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
               `Booking ID: #${booking.id}\n` +
               `Hotel Branch: ${branchName}\n` +
               ...
               `Phone: +256787437223\n` +
               `WhatsApp: +256787437223\n` +
               `Email: info@paradisehotel.com\n\n` +
               `We look forward to welcoming you!\n\n` +
               `Best regards,\n` +
               `Paradise Hotel ${branchName} Team\n` +
               `🏨 Your Home Away From Home`;
    } else {
        subject = `Booking Cancelled - Paradise Hotel ${branchName} - Booking #${booking.id}`;
        body = `Dear ${booking.name},\n\n` +
               `We regret to inform you that your booking at Paradise Hotel ${branchName} has been cancelled.\n\n` +
               `BOOKING DETAILS:\n` +
               `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
               `Booking ID: #${booking.id}\n` +
               `Hotel Branch: ${branchName}\n` +
...
function filterBookings() {
    ...
    // Filter by branch
    if (branchFilter !== 'all') {
        bookings = bookings.filter(b => (b.branch || 'Kampala') === branchFilter);
    }
